...
needToUpdate (PredictedPool (Pool _ b (FullTxOut q w e r t))) newGix = do
...
pessimistic consistentChain predictedPool confirmedPool = do
    if consistentChain then needToUpdate predictedPool (gId $ confirmed confirmedPool) else pure $ confirmed confirmedPool
