...
mkOrdersExecutor backlog transactions explorer resolver poolActions = do
  MakeLogging{..} <- askContext
  txRefsCfg       <- askContext
  logging         <- forComponent "OrdersExecutor"
  pure $ OrdersExecutor
    { run = run' logging txRefsCfg backlog transactions explorer resolver poolActions
    }
...
run'
  ...
  -> PoolActions
  -> s m ()
run' logging@Logging{..} txRefs backlog@BacklogService{..} txs explorer resolver poolActions =
