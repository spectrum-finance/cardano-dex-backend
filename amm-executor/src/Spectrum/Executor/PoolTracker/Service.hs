...
import Spectrum.Executor.Data.PoolState (Pool, Predicted (Predicted), Confirmed (Confirmed), Unconfirmed (Unconfirmed))
import Spectrum.Executor.PoolTracker.Data.Traced (Traced (Traced))
...
resolvePool'
  ...
  -> PoolId
  -> m (Maybe Pool)
resolvePool' pools@Pools{..} id = do
  confirmedM <- getLastConfirmed id
  unconfirmedM <- getLastUnconfirmed id
  predictedM <- getLastPredicted id
  case (confirmedM, unconfirmedM, predictedM) of
    (Just (Confirmed confirmedPool), _, Just (Predicted predictedPool@(OnChain predictedOut _))) -> do
      ...
      predictionIsValid <- if predictionIsAnchoringPoint then pure True else trace pools (fullTxOutRef predictedOut) (fullTxOutRef anchoringPoint)
...
invalidatePool'
  ...
  -> Pool
  -> m ()
invalidatePool' Pools{..} (OnChain FullTxOut{..} Core.Pool{..}) = invalidate poolId fullTxOutRef
